...
def compute_quality_flags(df: pd.DataFrame, summary: DatasetSummary, missing_df: pd.DataFrame,
    *, zero_share_threshold: float = 0.6, high_cardinality_threshold: int = 50,
) -> Dict[str, Any]:
    """
    ...
    if not df.empty:
        for name in df.columns:
            if int(df[name].nunique(dropna=False)) <= 1:
                constant_cols.append(name)
    flags["has_constant_columns"] = len(constant_cols) > 0
    flags["constant_columns"] = constant_cols

    # с большим числом уникальных значений
    high_card_cols = []
    if not df.empty:
        for name in df.columns:
            s = df[name]
            if ptypes.is_object_dtype(s) or isinstance(s.dtype, pd.CategoricalDtype):
                if int(s.nunique(dropna=True)) > high_cardinality_threshold:
    ...
    if not df.empty:
        for name in df.columns:
            n = name.strip().lower()
            if not (n == "id" or n.endswith("_id")):
                continue
            s = df[name].dropna()
            if not s.empty and bool(s.duplicated().any()):
                id_dup_cols.append(name)
    flags["has_suspicious_id_duplicates"] = len(id_dup_cols) > 0
    flags["suspicious_id_columns"] = id_dup_cols
    ...
    flags["many_zero_columns"] = zero_cols
