...
def report(
    ...
    title: Optional[str] = typer.Option(None, help="Заголовок отчёта (первая строка # ...)."),
    min_missing_share: float = typer.Option(0.1, help="Порог доли пропусков: выше — колонка считается проблемной."),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        report_title = title if title else "EDA-отчёт"
        ...
        f.write(
            ...
            f"{quality_flags.get('constant_columns', [])}\n"
        )
        f.write(
            f"- Высокая кардинальность категорий): "
            f"**{quality_flags.get('has_high_cardinality_categoricals', False)}** "
            f"{quality_flags.get('high_cardinality_categoricals', [])}\n"
        )
        f.write(
            f"- Дубликаты в id-колонках: **{quality_flags.get('has_suspicious_id_duplicates', False)}** "
            f"{quality_flags.get('suspicious_id_columns', [])}\n"
        )
        f.write(
            f"- Много нулей: **{quality_flags.get('has_many_zero_values', False)}** "
            f"{quality_flags.get('many_zero_columns', [])}\n\n"
        )
